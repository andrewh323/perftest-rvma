#!/bin/bash
#SBATCH --job-name=rsocket_lat_test
...
# Run server
ssh $server "export LD_LIBRARY_PATH=$RDMA_CORE_LIB:\$LD_LIBRARY_PATH; timeout 480 $PATH_TO_BIN/rsocket_server_lat &> $SERVER_OUT_PATH" &
server_pid=$!
...
# Run client
status=$(ssh $client "export LD_LIBRARY_PATH=$RDMA_CORE_LIB:\$LD_LIBRARY_PATH; timeout 480 $PATH_TO_BIN/rsocket_client_lat $server_ip &> $CLIENT_OUT_PATH; echo \$?" | tail -n 1)
client_pid=$!
