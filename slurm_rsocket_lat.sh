...
# Run server
ssh $server "timeout 480 $PATH_TO_BIN/rsocket_server_lat &> $SERVER_OUT_PATH" &
server_pid=$!
...
# Run client
status=$(ssh $client "timeout 480 $PATH_TO_BIN/rsocket_client_lat $server_ip &> $CLIENT_OUT_PATH; echo $?" | tail -n 1)
client_pid=$!
