#!/bin/bash
#SBATCH --job-name=rvma_lat_testing
...
# Run on server
ssh $server "$PATH_TO_BIN/ib_write_lat -s $SIZE &> $SERVER_OUT_PATH" &
server_pid=$!
...
# Then run on client
ssh $client "$PATH_TO_BIN/ib_write_lat $server_ip -s $SIZE &> $CLIENT_OUT_PATH"
client_pid=$!
