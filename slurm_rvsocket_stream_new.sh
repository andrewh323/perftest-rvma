...
if [ ! -f "$CSV_FILE" ]; then
    echo "timestamp,repetition,size_bytes,avg_send,mailbox_init,rvsocket,rvbind,rvconnect,rvaccept,qp_setup" > "$CSV_FILE"
fi
...
for REP in $(seq 1 $REPEATS); do
    ...
    for SIZE in "${SIZES[@]}"; do
        ...
        AVG_SEND=$(grep "Average time per message:"   "$CLIENT_OUT_PATH" | awk '{print $(NF-1)}')
        ...
        echo "$(date +"%H:%M:%S.%3N"),$REP,$SIZE,$AVG_SEND,$WINDOW_INIT,$RVSOCKET_SETUP,$RVBIND,$RVCONNECT,$RVACCEPT,$QPSETUP" >> "$CSV_FILE"
    done
