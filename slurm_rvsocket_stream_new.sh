...
if [ ! -f "$CSV_FILE" ]; then
    echo "timestamp,repetition,size_bytes,min_send,max_send,avg_send,std_dev_send,mailbox_init,rvsocket,rvbind,rvlisten,rvconnect,rvaccept,qp_setup" > "$CSV_FILE"
fi
...
for REP in $(seq 1 $REPEATS); do
    ...
    for SIZE in "${SIZES[@]}"; do
        ...
        echo "$(date +"%H:%M:%S.%3N"),$REP,$SIZE,$MIN_SEND,$MAX_SEND,$AVG_SEND,$STD_DEV_SEND,$WINDOW_INIT,$RVSOCKET_SETUP,$RVBIND,$RVLISTEN,$RVCONNECT,$RVACCEPT,$QPSETUP" >> "$CSV_FILE"
    done
