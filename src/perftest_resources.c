...
//THIS is the function to call the ibv_post_send
...
int alloc_ctx(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
    ...
    if(user_param->machine == SERVER) {
        ...
        *ctx->rvma_vaddr = 123;
        ...
        /* Initialize values inside the buffer if needed */
        memset(buffer, 0, rvma_buffer_size);

        /* Allocate and initialize notification buffer pointer */
        int* notifBuffPtr = malloc(sizeof(int));  // Allocate an int
        *notifBuffPtr = 0;
        int** notifBuffPtrAddr = malloc(sizeof(int*)); // Allocate an int*
        *notifBuffPtrAddr = notifBuffPtr;  // Point the allocated int* to the int
        ctx->rvma_notifBuffPtrAddr = (void **) notifBuffPtrAddr;

        int* notifLenPtr = malloc(sizeof(int));  // Allocate an int
        *notifLenPtr = 0;
        int** notifLenPtrAddr = malloc(sizeof(int*)); // Allocate an int*
        *notifLenPtrAddr = notifLenPtr;  // Point the allocated int* to the int
        ctx->rvma_notifLenPtrAddr = (void **) notifLenPtrAddr;
        ...
        if (status != RVMA_SUCCESS) {
            fprintf(stderr, "Failed to post RVMA buffer.\n");
            free(buffer);
            free(notifBuffPtr);
            free(notifLenPtr);
            return 1;
		...
		struct perftest_parameters *user_param)
{
    ...
    if(user_param->machine == SERVER && ctx->rvma_notifBuffPtrAddr != NULL){
        free(*ctx->rvma_notifLenPtrAddr); // Free the dynamically allocated int
        free(ctx->rvma_notifLenPtrAddr);
        ctx->rvma_notifLenPtrAddr = NULL;
    }

    if(user_param->machine == SERVER && ctx->rvma_notifBuffPtrAddr != NULL){
        free(*ctx->rvma_notifBuffPtrAddr); // Free the dynamically allocated int
        free(ctx->rvma_notifBuffPtrAddr);
        ctx->rvma_notifBuffPtrAddr = NULL;
    }
		...
		if ((user_param->verb == WRITE || user_param->verb == WRITE_IMM) && user_param->tst == LAT) {
			...
			if (user_param->has_payload_modification) {
				...
				//((char*)ctx->buf[qp_index])[i] = (char)rand();
                    if (user_param->machine == SERVER) {
                        ((char*)ctx->buf[qp_index])[i] = 'A';
                    } else if (user_param->machine == CLIENT) {
                        ((char*)ctx->buf[qp_index])[i] = 'Z';
                    }
                }
			}
		}
...
int run_iter_bw(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
    ...
    ctx->rvma_start_time.tv_sec = temp.tv_sec;
    ctx->rvma_start_time.tv_usec = temp.tv_usec;
	...
	while (totscnt < tot_iters  || totccnt < tot_iters ||
        ...
        gettimeofday(&temp, NULL);
        ctx->rvma_end_time.tv_sec = temp.tv_sec;
        ctx->rvma_end_time.tv_usec = temp.tv_usec;
	}
