...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < NUM_REPEATS; i++) {
        ...
        bandwidths[i] = (bytes_sent / elapsed) / (1024 * 1024 * 1024); // Calculate bandwidth in GiB/s
    }
    ...
    double avg_bandwidth = total_bw / NUM_REPEATS;

    double variance = 0.0;
    for (int i = 0; i < NUM_REPEATS; i++) {
        double diff = bandwidths[i] - avg_bandwidth;
        variance += diff * diff;
    }

    variance = variance / NUM_REPEATS;
    double stddev = sqrt(variance);

    printf("Average bandwidth: %.2f GiB/s\n", avg_bandwidth);
    printf("Standard deviation: %.2f GiB/s\n", stddev);
